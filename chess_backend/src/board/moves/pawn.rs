...
impl Board {
    fn pawn_capture(&mut self, pos_to: Position, pos_from: Position, piece: Piece, move_info: &Move) -> Result<UndoMove, MoveError> {
        let left = pos_to.file().offset(-1).ok_or(MoveError::Unknown(file!(), line!()))?;
        let right = pos_to.file().offset(1).ok_or(MoveError::Unknown(file!(), line!()))?;
        ...
        let piece_at = match piece_at {
            Some(p) => p,
            None => {
                if self.en_passant_square.is_some_and(|f| f == pos_to.to_square()) {
                    let step_inter: i8 = match piece {
                        Piece::White(_) => -1,
                        Piece::Black(_) => 1,
                    };
                    let mut pawn_pos = pos_to;
                    let position = pos_to.rank().offset(step_inter).expect("invalid en passant rank offset");
                    ...
                    self.update_cache();
                    return Ok(self.move_piece_unchecked(move_info));       
                }
                else {
    ...
    pub fn move_pawn(&mut self, move_info: Move) -> Result<UndoMove, MoveError> {
        ...
        // Double push
            ...
            let step_rank = piece.pawn_step(pos_from.rank())
                .ok_or(
                    MoveError::IllegalDoublePawnPush(pos_from, pos_to)
                )?; 
            ...
            undo = self.move_piece_unchecked(&move_info);
        ...
        else { return Err(MoveError::Unknown(file!(), line!())); } // some edge case i haven't thought about
        ...
        if pos_to.rank() == top_rank {
            self.promote_pawn(move_info)?;
        }

        Ok(undo)
    }

    fn promote_pawn(&mut self, move_info: Move) -> Result<(), MoveError> {
        let pos_to = move_info.move_to();
        let piece = move_info.promotion()
            .ok_or(MoveError::PromotionPieceMissing(move_info.move_from().to_position(), pos_to.to_position()))?;
        println!("{piece}");
        let pawn = self.get_cached_piece_at(&pos_to.to_position()).expect("Pawn should exist at pos_to");
